...
import {
  Phone, Mail, MessageSquare, Users, Clock, CheckCircle2,
  XCircle, AlertCircle, PhoneOff, PhoneMissed, CalendarClock, Plus,
} from "lucide-react";
...
const OUTCOMES = [
  ...
  { value: "left_voicemail", label: "Left Voicemail", icon: Phone, color: "text-blue-400" },
  { value: "hung_up", label: "Hung Up", icon: PhoneOff, color: "text-red-400" },
  { value: "thought_scam", label: "Thought It Was a Scam", icon: AlertCircle, color: "text-orange-400" },
  { value: "interested_sending_portal", label: "Interested — Sending Portal Link", icon: CheckCircle2, color: "text-emerald-400" },
  { value: "interested_thinking", label: "Interested — Needs to Think About It", icon: Clock, color: "text-amber-400" },
  { value: "not_interested", label: "Not Interested", icon: XCircle, color: "text-red-400" },
  { value: "wrong_number", label: "Wrong Number", icon: PhoneOff, color: "text-slate-400" },
  { value: "disconnected", label: "Number Disconnected", icon: PhoneOff, color: "text-slate-400" },
  { value: "call_back", label: "Need to Call Back", icon: CalendarClock, color: "text-purple-400" },
  { value: "signed_up", label: "Signed Up", icon: CheckCircle2, color: "text-emerald-400" },
];

const METHOD_ICONS = {
  phone: Phone, email: Mail, text: MessageSquare, in_person: Users,
};

const OUTCOME_BADGE_COLORS = {
  no_answer: "bg-slate-700 text-slate-300",
  left_voicemail: "bg-blue-500/15 text-blue-400",
  hung_up: "bg-red-500/15 text-red-400",
  thought_scam: "bg-orange-500/15 text-orange-400",
  interested_sending_portal: "bg-emerald-500/15 text-emerald-400",
  interested_thinking: "bg-amber-500/15 text-amber-400",
  not_interested: "bg-red-500/15 text-red-400",
  wrong_number: "bg-slate-700 text-slate-300",
  disconnected: "bg-slate-700 text-slate-400",
  call_back: "bg-purple-500/15 text-purple-400",
...
export default function ContactLogger({ caseId, caseData }) {
  ...
  const handleSubmit = () => {
    ...
    logMutation.mutate({
      case_id: caseId, contact_method: method, outcome,
      notes: notes || undefined, contacted_at: new Date().toISOString(),
          ...
          {attempts.length === 0 ? (
            ...
            <div className="space-y-3 max-h-80 overflow-y-auto">
              {attempts.map((attempt) => {
                ...
                return (
                  <div key={attempt.id} className="flex gap-3 items-start border-b border-slate-700 last:border-0 pb-3 last:pb-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <Badge className={`text-xs border-0 ${OUTCOME_BADGE_COLORS[attempt.outcome] || "bg-slate-700 text-slate-300"}`}>
                          {outcomeLabel}
                        </Badge>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Any additional notes..." rows={3} className="mt-1 resize-none" />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowDialog(false)}>Cancel</Button>
            <Button onClick={handleSubmit} disabled={logMutation.isPending} className="bg-emerald-600 hover:bg-emerald-700">
